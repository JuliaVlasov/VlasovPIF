...
async_mpi=true; # prefer Non-blocking MPI communication
...
if mpi_rk==0
  ...
  windowp = glscreen(color = RGBA(0f0, 0f0, 0f0, 0f0));
  @async renderloop(windowp)

  # 3d view into the particle coordinates
  pview3d(x1,x2,x3,v1,v2,v3)=(Float32.(x1),Float32.(v1),Float32.(v2))

# GeometryTypes.scalematrix(Vec3f0(5f0))

  particle_size=0.03f0
  pshape=GeometryTypes.Sphere{Float32}(Vec3f0(0), particle_size)

  #Scale the particle shape ot a siz
  pshape=GLAbstraction.scalematrix(Vec3f0(particle_size)*2 )*
                  GLNormalMesh(loadasset("cat.obj"))

  # _view(particle_vis, camera = :perspective)
  color = fill(RGBA{Float32}(1., 1., 1., 0.8),Np)

  hull=AABB(Vec3f0(minimum.(pview3d(x1n,x2n,x3n,v1n,v2n,v3n) )...),
          Vec3f0(minimum.(pview3d(x1n,x2n,x3n,v1n,v2n,v3n) )...))

  glparticles = visualize( (   pshape,
               GeometryTypes.Point3f0.(pview3d(x1n,x2n,x3n,v1n,v2n,v3n)... )),
               hull =hull,
              color=color, billboard = true).children[]
_view(glparticles,windowp,camera = :perspective)


# Update particles
GLAbstraction.set_arg!(glparticles, :position, GeometryTypes.Point3f0.(
             pview3d(x1n,x2n,x3n,v1n,v2n,v3n)... ) )
