...
struct pif3d{T}
  ...
  function pif3d{T}(Nx1::Int,Nx2::Int,Nx3::Int,
  ...
  function pif3d{T}(Nx1::Int,Nx2::Int,Nx3::Int,L::T) where T
     pif3d(Nx1,Nx2,Nx3,L,L,L)
  end
end
# Outer constructor to infer type T directly from given length
pif3d(Nx1::Int,Nx2::Int,Nx3::Int,L1::T,L2::T,L3::T) where
      {T<:AbstractFloat}=pif3d{T}(Nx1,Nx2,Nx3,L1T,L2::T,L3::T)
pif3d(Nx1::Int,Nx2::Int,Nx3::Int,L::T) where
            {T<:AbstractFloat}=pif3d{T}(Nx1,Nx2,Nx3,L)
...
function eval_basis2{T}(self::pif3d, x1n::T,x2n::T,  x3n::T,
...
@inline @inbounds function eval_basis{T}(self::pif3d, x1n::T,x2n::T,  x3n::T,
                            psin::Array{Complex{T},3})
...
@inline @inbounds function eval_basis{T}(self::pif3d{T}, x1n::T,x2n::T,  x3n::T,
                            psin::Array{Complex{T},3})
...
function eval_basis_defint{T}(self::pif3d{T}, x1n::T,x2n::T, x3n::T,x_n::T,
...
# The other components remain constant
function eval_basis_int{T}(self::pif3d{T}, x1n::T,x2n::T, x3n::T,
                          axis::Int,psin::Array{Complex{T},3})
...
@inbounds function eval_scalar{T}(self::pif3d, psin::Array{Complex{T},3},
                   F::Array{Complex{T},3} )
  # vali::Complex{T}=0
  # val::T=0
  # for kdx=1:self.N3
...
@inbounds function eval_scalar2{T}(self::pif3d, psin::Array{Complex{T},3},
                     F::Array{Complex{T},3} )
...
# evaluate a scalar function with coefficients F
@inbounds function eval_scalar_gradient{T}(self::pif3d,
          psin::Array{Complex{T},3},
...
function eval_scalar_gradient{T}(self::pif3d,
          psin::Array{Complex{T},3},
                        F::Array{Complex{T},3})
...
# function set_basis_int{T}(self::pif3d, psi::Array{Complex{T},3}, x::T,  axis::Int )
#      psi_int=copy(psi)
#      K=getK(self,axis)
#
#      if (axis==1)
...
# end
# using Devectorize
function integrate_Hp{T}(self::pif3d{T},x1n::Array{T,1},x2n::Array{T,1},
                       x3n::Array{T,1},
...
function push_Hp1{T}(self::pif3d, x1::T, x2::T, x3::T, v1::T, v2::T , v3::T, q::T, m::T, w::T,
...
function push_Hp1{T}(self::pif3d, x1::T, x2::T, x3::T, v1::T, v2::T , v3::T, q::T, m::T, w::T,
  ...
  function push_Hp1{T}(self::pif3d,
...
function push_Hp2{T}(self::pif3d, x1::T, x2::T, x3::T, v1::T, v2::T , v3::T, q::T, m::T, w::T,
            psi0::Array{Complex{T},3},  dt::T,
...
function push_Hp3{T}(self::pif3d, x1::T, x2::T, x3::T, v1::T, v2::T , v3::T, q::T, m::T, w::T,
            psi0::Array{Complex{T},3},  dt::T,
...
#rho from particles
@inbounds function accum_osde2{T <:AbstractFloat}(self::pif3d{T},
...
#rho from particles
function accum_osde{T <:AbstractFloat}(self::pif3d{T},
           x1n::Array{T,1},x2n::Array{T,1},  x3n::Array{T,1},
           wn::Array{T,1})
  Np=length(x1n)
  @assert Np==length(x2n)
...
function accum_osde_var{T}(self::pif3d{T},
...
@inbounds function accum_basis{T}(self::pif3d{T}, psin::Array{Complex{T},3},
...
function accum_osde_int{T <:AbstractFloat}(self::pif3d{T},
           x1n::Array{T,1},x2n::Array{T,1},  x3n::Array{T,1},
...
@inbounds function eval_scalar{T}(self::pif3d,
           x1n::Array{T,1},x2n::Array{T,1}, x3n::Array{T,1},
...
function eval_vectorfield{T}(self::pif3d,
           x1n::Array{T,1},x2n::Array{T,1}, x3n::Array{T,1},
...
function getK(self::pif3d,axis::Int)
  if axis==1
...
function solve_poisson{T}(self::pif3d{T}, rho::Array{Complex{T},3})
...
function solve_gauss{T}(self::pif3d{T}, rho::Array{Complex{T},3} )
  E1=similar(rho)
  E2=similar(rho)
  E3=similar(rho)
...
function gauss_error{T}(self::pif3d{T}, rho::Array{Complex{T},3}, E1,E2,E3 )
  K1,K2,K3=getK2(self)
  rho2=similar(rho)
  rho2[:]=rho[:]
...
function solve_gauss!{T}(self::pif3d{T}, rho::Array{Complex{T},3}, E1,E2,E3   )
...
function gradient{T}(self::pif3d{T}, F::Array{Complex{T},3})
  K1,K2,K3=getK2(self)
...
import Base: div
function div{T}(self::pif3d{T}, F::Array{Complex{T},3})
...
function H1seminorm{T}(self::pif3d{T},F::Array{Complex{T},3} )
  K1,K2,K3=getK2(self)
...
function L2norm{T}(self::pif3d{T},F::Array{Complex{T},3}  )
  L2=sum(abs.(F[:]).^2)*2.0- sum((abs.(F[pm.N1n,:,:]).^2)[:])
  L2*=self.L1*self.L2*self.L3
  return L2
end

import Base.LinAlg: dot
function dot{T}(self::pif3d{T}, A::Array{Complex{T},3},B::Array{Complex{T},3})
    ab=(sum(real(conj(A[2:end,:,:])).*real(B[2:end,:,:]))-sum(imag(conj(A[2:end,:,:])).*imag(B[2:end,:,:])))*2
          +imag(sum(conj(A[pm.N1n,:,:]).*B[pm.N1n,:,:]))
    return ab*self.L1*self.L2*self.L3
end


# Integrate Ampere equation from 0 to dt
function integrate_H_B{T}(self::pif3d{T}, E1::Array{Complex{T},3},
...
# Integrate Ampere equation from 0 to dt
function integrate_Faraday{T}(self::pif3d{T}, E1::Array{Complex{T},3},
           E2::Array{Complex{T},3},E3::Array{Complex{T},3},
           B1::Array{Complex{T},3},B2::Array{Complex{T},3},
...
@inbounds function integrate_H_E{T}(self::pif3d{T}, E1::Array{Complex{T},3},
           E2::Array{Complex{T},3},E3::Array{Complex{T},3},
...
function test_eval_basis{T}(pm::pif3d{T}, N::Int )
...
function integrate_Hp_boris_exp{T}(self::pif3d{T},x1n::Array{T,1},x2n::Array{T,1},
                       x3n::Array{T,1},
                       v1n::Array{T,1},v2n::Array{T,1}, v3n::Array{T,1},
                      qn::Array{T,1}, mn::Array{T,1},wn::Array{T,1},
          J1::Array{Complex{T},3},J2::Array{Complex{T},3},J3::Array{Complex{T},3},
...
function integrate_Hp_split_sym{T}(self::pif3d{T},x1n::Array{T,1},x2n::Array{T,1},
                       x3n::Array{T,1},
                       v1n::Array{T,1},v2n::Array{T,1}, v3n::Array{T,1},
                      qn::Array{T,1}, mn::Array{T,1},wn::Array{T,1},
          J1::Array{Complex{T},3},J2::Array{Complex{T},3},J3::Array{Complex{T},3},
...
function integrate_Hp_midpoint{T}(self::pif3d{T},x1n::Array{T,1},x2n::Array{T,1},
                       x3n::Array{T,1},
                       v1n::Array{T,1},v2n::Array{T,1}, v3n::Array{T,1},
                      qn::Array{T,1}, mn::Array{T,1},wn::Array{T,1},
          J1::Array{Complex{T},3},J2::Array{Complex{T},3},J3::Array{Complex{T},3},
...
#
#
# function integrate_Hp1{T}(self::pif3d, d_E1::Array{Complex{T},3},
#           B2,B3
#            x1n::Array{T,1},x2n::Array{T,1}, x3n::Array{T,1},
