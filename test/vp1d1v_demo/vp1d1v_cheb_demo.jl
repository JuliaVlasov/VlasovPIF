...
function f(x,v)
  return exp(-0.5.*v.^2)/sqrt(2*pi).*(1+0.5.*cos(k0*x))
end
function g(x,v)
...
# end
# Nf=2
# p = sortperm(DE);DV=DV[p[1:Nf]]; DE=DE[p[1:Nf]]
...
@inbounds for tdx=1:Nt
  ...
  @inbounds for rkdx=1:length(rksd)
    ...
    Phi=QR \[zeros(Float64,deg);0.0;0.0;rho]
    Phi.coefficients=Phi.coefficients[1:Nx+2]


    #E=-rho./(-im*kx)
    if rkdx==1
      fieldenergy[tdx]=coefficients(IN*(D1*Phi)^2)[1]/2*(L/2)
      #fieldenergy[tdx]=dot(Phi.coefficients[1:Nx],rhs)/2*(L/2)/(2/L)
      kineticenergy[tdx]=0.5.*sum(vk.^2.*wk )/Np
      momentum[tdx]=sum(vk.*wk)/Np
    end
    Phic=-coefficients(Phi)/(2/L)
    vk.+=dt*rksc[rkdx]*chebTdx_series(xk/L*2-1.0,Phic )*(2/L)
    xk.+=dt*rksd[rkdx].*vk
    xk=mod(xk,L)
  end
...
prefix="./plots/cheb_landau_strong"
...
# using JLD
# @load "plots/landau_strong_results.jld"
